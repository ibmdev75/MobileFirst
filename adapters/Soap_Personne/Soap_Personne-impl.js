...
function DonneesClientServiceImplService_rechercherSouscripteurClient(params, headers){
	var soapEnvNS = '';
    ...
    var mappings = {
		roots: {
			'rechercherSouscripteurClient': { nsPrefix: 'tns', type: 'tns:rechercherSouscripteurClient' }				
		},
		
		types: {
			'tns:critereRechercherSouscripteurClient': {
				children: [
					{'noSocietaire': {  }},	
					{'cdCanalAcces': {  }}	
				]
			...
			'tns:rechercherSouscripteurClient': {
				children: [
					{'critereRechercherSouscripteurClient': { type: 'tns:critereRechercherSouscripteurClient' }},	
					{'criterePagination': { type: 'ns1:criterePagination' }}	
				]
			}
		}
	};
    var namespaces = 'xmlns:ns1="http://smabtp.fr/model/CommonTypes/1.0" xmlns:tns="http://smabtp.fr/service/transverse/DonneesClientService/1.0" ';
    var request = buildBody(params, namespaces, mappings, soapEnvNS);
    var soapAction = 'http://TRANSVERSE.smabtp.fr/RECHERCHER_SOUSCRIPTEURS_CLIENT/1.0';
    return invokeWebService(request, headers, soapAction);
}


function DonneesClientServiceImplService_lireInformationsPersonne(params, headers){
	var soapEnvNS = '';
    // The value of 'soapEnvNS' was set based on the version of the SOAP to be used (i.e. 1.1 or 1.2).
    soapEnvNS = 'http://schemas.xmlsoap.org/soap/envelope/';
    ...
    var mappings = {
		roots: {
			'lireInformationsPersonne': { nsPrefix: 'tns', type: 'tns:lireInformationsPersonne' }				
		},
		
		types: {
			'tns:lireInformationsPersonne': {
				children: [
					{'critereLireInformationsPersonne': { type: 'tns:critereLireInformationsPersonne' }}	
				]
			},

			'tns:critereLireInformationsPersonne': {
				children: [
					{'noSocietaire': {  }},	
					{'noSIRET': {  }},	
					{'noGRC': {  }},	
    ...
    var namespaces = 'xmlns:ns1="http://smabtp.fr/model/CommonTypes/1.0" xmlns:tns="http://smabtp.fr/service/transverse/DonneesClientService/1.0" ';
    var request = buildBody(params, namespaces, mappings, soapEnvNS);
    var soapAction = 'http://TRANSVERSE.smabtp.fr/LIRE_INFORMATIONS_PERSONNE/1.0';
    return invokeWebService(request, headers, soapAction);
}
...
function invokeWebService(body, headers, soapAction){
    var input = {
        ...
        path : '/ba-web/donneesClient-service',
        body: {
